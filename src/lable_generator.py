...
def create_dataset(image_dir, output_label_dir, output_image_dir, detector, classifier, copy_images=True):
    """
    处理图像并生成标签
    ...
    Args:
        ...
        output_image_dir: 输出图像目录（当 copy_images=True 时使用）
        detector: 装甲板检测器
        classifier: 数字分类器
        copy_images: 是否复制图像到新目录
            - True: 复制图像到 output_image_dir，标签保存到 output_label_dir
            - False: 不复制图像，直接在原始图像所在目录生成标签文件
    """
    ...
    for image_file in tqdm(image_files, desc="Processing Dataset"):
        ...
        if copy_images:
            ...
            # 不复制图像，直接在原始图像所在目录生成标签文件
            label_file_path = image_file.parent / (image_file.stem + ".txt")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="使用传统算法自动标注装甲板数据，标注文件",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
使用示例:
  # 方式1: 复制图像到新目录（默认）
  python lable_generator.py --image-dir ./raw_images --output-dir ./output

  # 方式2: 直接在原始目录生成标签（不复制图像）
  python lable_generator.py --image-dir ./raw_images --no-copy

  # 自定义模型路径和二值化阈值
  python lable_generator.py --image-dir ./raw_images --model ./model/cnn.onnx --label ./model/label.txt --thresh 120
        """
    )
    ...
    # 可选参数 - 模型相关
    parser.add_argument("--model", "-m", type=str, default="model/cnn.onnx",
                        help="ONNX模型路径 (默认: model/cnn.onnx)")
    parser.add_argument("--label", "-l", type=str, default="model/label.txt",
                        help="标签文件路径 (默认: model/label.txt)")
    ...
    parser.add_argument("--no-copy", action="store_true",
                        help="不复制图像，直接在原始图像目录生成标签文件")
    
    # 可选参数 - 检测器参数
    parser.add_argument("--thresh", "-t", type=int, default=100,
                        help="二值化阈值 (默认: 100)")
    ...
    # 初始化检测器和分类器
    detector = TraditionalArmorDetector(binary_thresh=args.thresh)
    classifier = NumberClassifier(args.model, args.label)
    
    # 根据是否复制图像选择不同的输出方式
    if args.no_copy:
        # 方式2: 不复制图像，直接在原始目录生成标签
        print(f"\n处理图像目录: {args.image_dir}")
        print("模式: 直接在原始目录生成标签文件（不复制图像）")
        create_dataset(
            image_dir=args.image_dir,
            output_label_dir="",  # 此参数在 copy_images=False 时不会被使用
            output_image_dir="",  # 此参数在 copy_images=False 时不会被使用
            detector=detector,
            classifier=classifier,
            copy_images=False
        )
    else:
        # 方式1: 复制图像到新目录，label和image放在同一目录下
        output_dir = args.output_dir
        
        print(f"\n处理图像目录: {args.image_dir}")
        print(f"输出目录: {output_dir}")
        print("模式: 复制图像到新目录（图像和标签在同一文件夹）")
        
        create_dataset(
            image_dir=args.image_dir,
            output_label_dir=output_dir,  # 标签和图像放在同一目录
